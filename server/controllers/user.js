...
exports.createUser = (req, res, next) => {
  if (!req.errors.isEmpty) {
    console.log("req.errors.errors", JSON.stringify(req.errors));
...
exports.updateUser = (req, res, next) => {
  ...
  User.findById(userId)
    ...
    .then((user) => {
      ...
      bcrypt.hash(password.trim(), 12).then((hashedPass) => {
        Object.assign(currentUser, {
          username,
          email,
          password: hashedPass,
...
exports.setAvatar = (req, res, next) => {
  const photoUrl = `${
    req.file.path.replace(/\\/g, "/").split(".")[0]
  }-optimized.jpeg`;
  let pastPhotoUrl;
  User.findById(req.userId)
    .then((user) => {
      ...
      minifyAndResize(req.file, 160).catch(() => {
